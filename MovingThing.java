import java.awt.Color;
import java.awt.Graphics;

public abstract class MovingThing implements Moveable, Collidable
{
  private int xPos;
  private int yPos;
  private int width;
  private int height;

  public MovingThing()
  {
    xPos = 10;
    yPos = 10;
    width = 10;
    height = 10;
  }

  public MovingThing(int x, int y)
  {
    xPos = x;
    yPos = y;
    width = 75;
    height = 75;
  }

  public MovingThing(int x, int y, int w, int h)
  {
    xPos = x;
    yPos = y;
    width = w;
    height = h;
  }

  public void setPos(int x, int y)
  {
    xPos = x;
    yPos = y;
  }

  public void setX(int x)
  {
    xPos = x;
  }

  public void setY(int y)
  {
    yPos = y;
  }

  public int getX()
  {
    return xPos;   //finish this method
  }

  public int getY()
  {
    return yPos;  //finish this method
  }

  public void setWidth(int w)
  {
    width = w;
  }

  public void setHeight(int h)
  {
    height = h;
  }

  public int getWidth()
  {
    return width;  //finish this method
  }

  public int getHeight()
  {
    return height;  //finish this method
  }

  public boolean didCollideLeft(Object obj) {
    MovingThing m = (MovingThing) obj;
    return getX() < m.getX() + m.getWidth();
  }

  public boolean didCollideRight(Object obj) {
    MovingThing m = (MovingThing) obj;
    return getX() + getWidth() >= m.getX();
  }

  public boolean didCollideTop(Object obj) {
    MovingThing m = (MovingThing) obj;
    return getY() >= m.getY() && getY() <= m.getY() + getHeight();
  }

  public boolean didCollideBottom(Object obj) {
    MovingThing m = (MovingThing) obj;
    return getY() + getHeight() >= m.getY() && getY() + getHeight() <= m.getHeight() + m.getY();
  }

  public abstract void move(String direction);
  public abstract void draw(Graphics window);

  public String toString()
  {
    return getX() + " " + getY() + " " + getWidth() + " " + getHeight();
  }
}
